...
def test_exact_match_prefilter() -> None:
    ...
    def _match_fn(_construct: str, _h: dict, _l: dict) -> QuoteMatchDecision:
        return QuoteMatchDecision(match=False, reason="should not be called", method="stub")

    comparator = PychometricsComparator(api_key="test", match_model="test", match_fn=_match_fn)
...
def test_llm_matching_stub() -> None:
    ...
    human = {
        "document_id": "doc_2",
        "instances": [
            {"construct": "Growth Mindset", "quote": "I can improve", "speaker_id": "P1"}
        ],
    }
    llm = {
        "document_id": "doc_2",
        "instances": [
            {"construct": "Growth Mindset", "quote": "I can get better", "speaker_id": "P1"}
        ],
    }
...
def test_directory_comparison(tmp_path) -> None:
    ...
    human_doc = {
        "document_id": "doc_a",
        "instances": [
            {"construct": "A", "quote": "human only", "speaker_id": "P1"}
        ],
    }
    llm_doc = {
        "document_id": "doc_b",
        "instances": [
            {"construct": "B", "quote": "llm only", "speaker_id": "P2"}
        ],
    }
